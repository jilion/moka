(function() {
  var BALANCED_PAIRS, COLLECTION_CLOSE, COLLECTION_OPEN, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_BLOCK, IMPLICIT_CALL, IMPLICIT_END, IMPLICIT_FUNC, IMPLICIT_UNSPACED_CALL, INVERSES, LINEBREAKS, SINGLE_CLOSERS, SINGLE_LINERS, left, pair, rite, _i, _j, _k, _len, _len2, _len3, _ref, _results, _results2;
  ...
  exports.Rewriter = function() {
    ...
    Rewriter.prototype.addImplicitBraces = function() {
      ...
      action = function(token, i) {
        ...
        return this.tokens.splice(i, 0, ['}', value, token[2]]);
      };
      return this.scanTokens(function(token, i, tokens) {
    ...
    Rewriter.prototype.rewriteListedFnIndents = function() {
      var count, gen, stack;
      stack = [];
      gen = [];
      count = 0;
      return this.scanTokens(function(token, i, tokens) {
        var coll, glyph, id, last, next, skip, _ref, _ref2, _ref3;
        id = token[0];
        _ref = stack.slice(stack.length - 3), coll = _ref[0], glyph = _ref[1], last = _ref[2];
        glyph = (_ref2 = glyph != null ? glyph[0] : void 0) === '->' || _ref2 === '=>';
        skip = 1;
        if (id === ',' && (last != null ? last[0] : void 0) === 'INDENT' && glyph) {
          tokens.splice(i, 0, ['OUTDENT', 2, last[2]]);
          count += 1;
          next = tokens[i + 2];
          if (((coll != null ? coll[0] : void 0) === (_ref3 = next != null ? next[0] : void 0) && _ref3 === '{') && next[1].generated) {
            tokens.splice(i + 2, 1);
            gen.push(true);
          }
          skip = 2;
        } else if (__indexOf.call(COLLECTION_OPEN, id) >= 0) {
          stack.push(token);
          if (id === '{') {
            gen.push(false);
          }
        } else if (__indexOf.call(COLLECTION_CLOSE, id) >= 0) {
          if (count && id !== 'OUTDENT') {
            if (id === '}' && token[1].generated) {
              if (gen[gen.length - 1]) {
  ...
  COLLECTION_CLOSE = (function() {
    _results2 = [];
    for (_k = 0, _len3 = BALANCED_PAIRS.length; _k < _len3; _k++) {
      pair = BALANCED_PAIRS[_k];
      if (pair[1]) {
        _results2.push(pair[1]);
      }
    }
    return _results2;
  }()).concat(['FORIN', 'FOROF']);
